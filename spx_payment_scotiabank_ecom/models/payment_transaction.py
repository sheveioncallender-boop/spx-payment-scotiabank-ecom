...
class PaymentTransaction(models.Model):
    ...
    scotia_gateway_store_id = fields.Char(string='Gateway Store ID Used', readonly=True, copy=False)
    ...
    def _get_specific_rendering_values(self, processing_values):
        ...
        store_id = provider._scotia_get_store_id()
        ...
        self.write({
            ...
            'scotia_gateway_store_id': store_id,
        })
    ...
    def _apply_updates(self, payment_data):
        ...
        response_hash_valid = provider._scotia_validate_response_hash(
            payment_data,
            fallback_store_id=self.scotia_gateway_store_id or provider._scotia_get_store_id(),
        )
        if provider.scotia_require_response_hash and not response_hash_valid:
            ...
            self._scotia_message_post(_(
                'Scotiabank eCom+ response hash validation failed.<br/>Status: %(status)s<br/>Amount: %(amount)s %(currency)s<br/>Transaction ID: %(txid)s'
            ) % {
                'status': status or payment_data.get('approval_code') or '',
                'amount': payment_data.get('chargetotal') or '',
                'currency': payment_data.get('currency') or '',
                'txid': payment_data.get('ipgTransactionId') or payment_data.get('refnumber') or '',
            })
            return
        ...
        if is_approved:
            # Ensure Odoo accounting has the provider's inbound payment method line
            # before _set_done triggers standard post-processing/account payment creation.
            try:
                provider._scotia_ensure_account_payment_method_line()
            except Exception:
                _logger.exception('Scotiabank eCom+: could not ensure accounting payment method line before confirming transaction.')
            self._set_done()
            self._scotia_message_post(_(
