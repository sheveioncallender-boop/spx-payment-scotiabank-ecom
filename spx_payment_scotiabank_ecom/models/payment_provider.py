...
class PaymentProvider(models.Model):
    ...
    def create(self, vals_list):
        ...
        providers.filtered(lambda p: p.code == SCOTIABANK_ECOM_CODE)._scotia_ensure_payment_method()
        return providers

    def write(self, vals):
        res = super().write(vals)
        self.filtered(lambda p: p.code == SCOTIABANK_ECOM_CODE)._scotia_ensure_payment_method()
        return res

    def _scotia_ensure_payment_method(self):
        """Ensure the standard Odoo Card payment method is linked to this provider.

        Odoo 19 uses payment.method records to decide which payment options appear
        on the checkout/payment form. Some custom providers can install without any
        method linked, which makes the Payment Methods smart button empty and can
        hide the provider at checkout.
        """
        ...
        for provider in self:
            if card_method not in provider.payment_method_ids:
                provider.payment_method_ids = [Command.link(card_method.id)]
