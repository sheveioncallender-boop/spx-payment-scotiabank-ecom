...
class ScotiabankEcomController(http.Controller):
    ...
    def scotiabank_return(self, **post):
        ...
        tx_model._process(SCOTIABANK_ECOM_CODE, payment_data)

        # Keep Odoo's standard post-payment flow.
        # For website checkout, /shop/payment/validate will perform the normal
        # website_sale validation and redirect the customer to the standard
        # /shop/confirmation thank-you page. For portal invoice/order payment links,
        # fall back to Odoo's generic payment status page.
